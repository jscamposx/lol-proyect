...
export const PlayerHeader = ({ player, onRefresh, lastUpdated, message }: { player: DashboardPlayer, onRefresh: () => void, lastUpdated: number|null, message: string|null }) => {
  ...
  return (
    <Card className="flex flex-col lg:flex-row items-center lg:items-stretch gap-5 sm:gap-6 p-4 sm:p-5 md:p-7 relative overflow-hidden">
      ...
      <div className="flex-1 text-center lg:text-left z-10 min-w-0">
        ...
        <h1 className="text-3xl sm:text-4xl md:text-5xl font-black text-white flex flex-col lg:flex-row items-center lg:items-baseline gap-1.5 sm:gap-2 mb-3 min-w-0">
        ...
        <p className="text-slate-400 text-sm flex items-center justify-center lg:justify-start gap-3 flex-wrap">
      ...
      <div className="flex flex-col gap-3 w-full lg:w-auto z-10 justify-center">
        <button
          onClick={onRefresh}
          className="group/btn whitespace-nowrap px-6 py-3.5 bg-linear-to-r from-violet-600 to-cyan-600 hover:from-violet-500 hover:to-cyan-500 text-white rounded-lg font-bold transition-all flex items-center justify-center gap-2 shadow-xl hover:shadow-violet-500/25 border border-white/10 w-full lg:w-auto"
        >
